...
class ResearchInteraction:
    ...
    clarification_responses: dict[str, str] | None = None
    current_question_index: int = 0
    enriched_query: str | None = None
    final_result: str | None = None
    report_data: Any | None = None  # Will hold ReportData object
    status: str = (
        "pending"  # pending, awaiting_clarifications, collecting_answers, researching, completed
    )
    ...
    def __str__(self):
        ...
        return (
            f"Query: {self.original_query}, Status: {self.status}, Questions: {questions_progress}"
        )
